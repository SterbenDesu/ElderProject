-- Phase 10 — Completion & dispute flow (NO money moves; Stripe is Phase 11).
--
-- This migration closes out an approved reservation and adds the admin dispute
-- review queue. It only ever sets STATUS fields; it never captures, releases, or
-- refunds money. Phase 11's Stripe logic will act on the payout/payment STATES we
-- set here (`ready_for_release`, `held_review`, `to_be_refunded`).
--
-- What changes vs. the earlier state-machine migration:
--   1. reservations gains `completed_at` / `disputed_at` timestamps.
--   2. payments enums gain the intermediate, money-not-yet-moved states:
--        payout_status  += 'ready_for_release', 'held_review'
--        payment_status += 'to_be_refunded'
--   3. notifications type += 'reservation_completed'.
--   4. refresh_reservation_progress() — time-based promotion on page load
--        (approved → in_progress → awaiting_confirmation), no background job.
--   5. transition_reservation() gains an optional p_detail and is rewritten so
--        `complete` sets payout_status='ready_for_release' (NOT released/paid),
--        and `report` stores the elder's issue text + flags the admin queue.
--   6. resolve_dispute() — ADMIN-ONLY dispute resolution (release | refund).
--   7. get_admin_disputes() — ADMIN-ONLY review queue read.
--
-- IDEMPOTENT (create-or-replace + drop-constraint-if-exists), so it doubles as the
-- runnable patch in SUPABASE_FIX_COMPLETION.sql.
