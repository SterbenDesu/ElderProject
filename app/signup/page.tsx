...
export default function SignupPage() {
  ...
  const [accountType, setAccountType] =
    useState<AccountType>("client_caregiver");
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (databaseResult.errorMessage) {
      setErrorMessage(
        `${databaseResult.errorMessage}. Please log in after confirming your email, then open the dashboard to retry profile setup.`,
      );
      setPassword("");
      setIsSubmitting(false);
      return;
    ...
    setSuccessMessage(
      "Signup complete. Your auth account, profile, and Terms/Privacy acceptance were saved. Check your email if Supabase email confirmation is enabled, then log in.",
    );
    setPassword("");
    setIsSubmitting(false);
  }
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[1fr_0.72fr]">
        <div className="rounded-[2rem] bg-white p-6 text-stone-700 shadow-sm ring-1 ring-stone-200">
          ...
          <p className="mt-4 text-lg leading-8">
            Choose whether you are joining as a client/caregiver or as a helper
            applicant. This creates your Supabase auth account and a matching
            database profile when the database schema is applied.
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-6 grid gap-5">
            ...
            <fieldset className="grid gap-3 rounded-3xl border border-stone-200 p-4">
              ...
              <label className="flex gap-3 rounded-2xl bg-cream px-4 py-3 text-sm font-semibold text-stone-700">
                ...
                <span>
                  Client/caregiver
                  <span className="block font-normal leading-6">
                    For elderly people or family caregivers looking for
                    non-medical support.
                  </span>
                </span>
              </label>
              <label className="flex gap-3 rounded-2xl bg-cream px-4 py-3 text-sm font-semibold text-stone-700">
                ...
                <span>
                  Helper applicant
                  <span className="block font-normal leading-6">
                    For independent helpers applying to offer non-medical
                    support later.
                  </span>
                </span>
              </label>
            ...
            <label className="flex gap-3 rounded-3xl border border-stone-200 bg-cream p-4 text-sm font-semibold text-stone-700">
              ...
              <span>
                ...
                .
                <span className="block font-normal leading-6">
                  Signup is disabled until these are accepted.
                </span>
              </span>
            </label>
        ...
        <aside className="rounded-[2rem] bg-sage p-6 text-stone-700">
          <h2 className="text-xl font-bold text-forest">
            Current signup scope
          </h2>
          <ul className="mt-4 space-y-3 leading-7">
            <li>
              • Account type and Terms/Privacy acceptance are saved to Supabase
              auth metadata and app database tables when allowed by RLS.
            </li>
            <li>
              • Basic client booking requests, helper applications, verified
              helper profile editing, and admin-controlled helper visibility are
              implemented; payments and booking assignment are not implemented
              yet.
            </li>
            <li>
              • No payments, medical services, or helper employment relationship
              are added.
            </li>
          </ul>
        </aside>
