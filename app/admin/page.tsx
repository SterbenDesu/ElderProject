...
type AdminPageStatus = "loading" | "signed-out" | "unconfigured" | "checking-profile" | "missing-profile" | "access-denied" | "admin" | "error";
...
const adminSections: DashboardSection[] = [
  { title: "Helper applications", description: "Review submitted helper applications and make basic verification decisions." },
  { title: "Users/profiles overview", description: "Placeholder for future support-safe profile lookup and account status tools." },
  { title: "Bookings", description: "Placeholder only. Booking management and booking payments are not implemented." },
  { title: "Disputes", description: "Placeholder for future complaint and dispute review workflows." },
  { title: "Audit logs", description: "Placeholder for future review of safety and moderation events." },
];
...
export default function AdminPage() {
  ...
  const loadApplications = useCallback(async () => {
    ...
    const result = await loadAdminHelperApplications(supabase);

    if (result.errorMessage) {
      setStatus("error");
      setMessage(`Could not load helper applications: ${result.errorMessage}. Confirm the helper_applications admin select policy is applied.`);
      setApplications([]);
      return;
    }
    ...
    setSelectedApplicationId((currentId) => {
      if (currentId && result.applications.some((application) => application.id === currentId)) {
        return currentId;
      }
  ...
  const checkAdminAccess = useCallback(async (currentUser: User) => {
    const { supabase, envError } = getSupabaseBrowserClient();
    ...
    if (profileResult.errorMessage) {
      setStatus("error");
      setMessage(`Could not load your profile for admin access: ${profileResult.errorMessage}. Confirm the profiles RLS policies are applied.`);
      setProfile(null);
      setApplications([]);
      return;
    }
    ...
    if (!profileResult.profile) {
      ...
      setMessage("Your auth account exists, but no profiles row was found. Create or repair the profile before using admin tools.");
      setProfile(null);
      setApplications([]);
      return;
    }
    ...
    if (profileResult.profile.role !== "admin") {
      ...
      setMessage("Access denied. Your profile role is not admin, so this page will not load helper application review data.");
      setApplications([]);
      return;
    }
  ...
  async function handleStatusChange(application: AdminHelperApplication, newStatus: AdminApplicationActionStatus) {
    const { supabase, envError } = getSupabaseBrowserClient();
    ...
    if (!user || !profile || profile.role !== "admin") {
      setActionError("Only signed-in admin users can review helper applications.");
      return;
    }
    ...
    if (result.errorMessage || !result.application) {
      setActionError(result.errorMessage ?? "The status update failed for an unknown reason.");
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-12 lg:px-8 lg:py-16">
      <p className="text-sm font-bold uppercase tracking-[0.2em] text-clay">Admin dashboard</p>
      <h1 className="mt-3 text-4xl font-bold tracking-tight text-forest sm:text-5xl">Helper application review</h1>
      <p className="mt-5 max-w-3xl text-lg leading-8 text-stone-700">
        Admin users can review helper applications, update review status, and approve a basic helper profile without making that helper publicly visible automatically.
      </p>
      ...
      {status === "loading" || status === "checking-profile" ? (
        <div className="mt-8 rounded-[2rem] bg-white p-6 text-stone-700 shadow-sm ring-1 ring-stone-200" role="status">
          {status === "loading" ? "Checking your admin session..." : "Checking your profile role..."}
        </div>
      ) : null}
      ...
      {status === "signed-out" ? (
        <div className="mt-8 rounded-[2rem] bg-white p-6 text-stone-700 shadow-sm ring-1 ring-stone-200">
          ...
          <p className="mt-4 text-lg leading-8">You need to log in with an admin account before viewing helper application review data.</p>
          {message ? <p className="mt-4 rounded-2xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{message}</p> : null}
          <Link href="/login" className="mt-6 inline-flex min-h-12 items-center rounded-full bg-forest px-5 py-3 font-semibold text-white transition hover:bg-stone-800">
            Login
          </Link>
        </div>
      ) : null}

      {status === "missing-profile" || status === "access-denied" || status === "error" ? (
        <div className="mt-8 rounded-[2rem] border border-amber-200 bg-amber-50 p-6 text-amber-900" role="alert">
          <h2 className="text-2xl font-bold">
            {status === "access-denied" ? "Access denied" : "Admin access needs attention"}
          </h2>
          <p className="mt-4 leading-7">{message}</p>
          <p className="mt-4 text-sm font-semibold leading-6">No admin application data is loaded for this account.</p>
        </div>
      ) : null}
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-[0.95fr_1.35fr]">
          <div className="space-y-5">
            <section className="rounded-[2rem] bg-white p-6 text-stone-700 shadow-sm ring-1 ring-stone-200">
              <h2 className="text-2xl font-bold text-forest">Dashboard sections</h2>
              <div className="mt-5 grid gap-3">
                {adminSections.map((section) => (
                  <article key={section.title} className="rounded-3xl border border-stone-200 p-4">
            ...
            <section className="rounded-[2rem] bg-white p-6 text-stone-700 shadow-sm ring-1 ring-stone-200">
              ...
              {emailWarning ? <p className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm font-semibold leading-6 text-amber-900">{emailWarning}</p> : null}

              {applications.length === 0 ? (
                <p className="mt-5 rounded-3xl bg-cream p-5 leading-7 text-stone-700">No helper applications are available for review yet.</p>
              ) : (
                  ...
                  {applications.map((application) => (
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <h3 className="font-bold text-forest">{application.full_name}</h3>
                      ...
                      <p className="mt-2 text-sm font-semibold text-stone-600">{application.city} · {formatDateTime(application.created_at)}</p>
                      {application.applicant_email ? <p className="mt-1 break-words text-sm text-stone-600">{application.applicant_email}</p> : null}
                      <p className="mt-3 text-sm leading-6 text-stone-600">{previewText(application.motivation)}</p>
                    </button>
                  ))}
                </div>
              )}
          ...
          <section className="rounded-[2rem] bg-white p-6 text-stone-700 shadow-sm ring-1 ring-stone-200">
            ...
            {!selectedApplication ? (
              <p className="mt-5 rounded-3xl bg-cream p-5 leading-7">Select an application to review, or wait for applicants to submit helper applications.</p>
            ) : (
              <article className="mt-5">
                <dl className="grid gap-4 rounded-3xl bg-cream p-5 text-sm">
                  ...
                  <div>
                    ...
                    <dd className="mt-1 break-words">{selectedApplication.applicant_email ?? "Not available through current profile read"}</dd>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <dd className="mt-1">{formatHelperApplicationStatus(selectedApplication.status)}</dd>
                  </div>
                  <div>
                    <dt className="font-bold text-forest">Created date</dt>
                    <dd className="mt-1">{formatDateTime(selectedApplication.created_at)}</dd>
                  </div>
                  <div>
                    <dt className="font-bold text-forest">Updated date</dt>
                    <dd className="mt-1">{formatDateTime(selectedApplication.updated_at)}</dd>
                  </div>
                </dl>
                ...
                <div className="mt-5 space-y-4">
                  <section className="rounded-3xl border border-stone-200 p-4">
                    ...
                    <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-stone-600">{selectedApplication.motivation}</p>
                  </section>
                  <section className="rounded-3xl border border-stone-200 p-4">
                    <h3 className="font-bold text-forest">Experience summary</h3>
                    <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-stone-600">{selectedApplication.experience_summary || "Not provided"}</p>
                  </section>
                  <section className="rounded-3xl border border-stone-200 p-4">
                    <h3 className="font-bold text-forest">Availability summary</h3>
                    <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-stone-600">{selectedApplication.availability_summary || "Not provided"}</p>
                  </section>
                </div>
                ...
                <div className="mt-6 rounded-3xl border border-clay/30 bg-cream p-5">
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    {(["under_review", "approved", "rejected"] as AdminApplicationActionStatus[]).map((nextStatus) => (
                      <button
                        key={nextStatus}
                        type="button"
                        onClick={() => handleStatusChange(selectedApplication, nextStatus)}
                        disabled={workingApplicationId === selectedApplication.id || selectedApplication.status === nextStatus}
                      ...
                      >
                        {workingApplicationId === selectedApplication.id ? "Saving..." : actionLabel(nextStatus)}
                      </button>
                    ))}
                  </div>
