-- Notification center + reservation read RPCs (Phase 8).
--
-- The notifications/chat/reservation TABLES, their RLS, and the state-machine
-- RPCs (create_reservation / transition_reservation) already exist from the
-- earlier migrations. This migration adds ONLY the read paths the UI needs:
--
--   get_my_notifications()   — the bell panel: a user's own notifications,
--                              newest first, enriched with the counterparty's
--                              PUBLIC-safe name (elder first name OR caregiver
--                              display name) and the related reservation status.
--   mark_notifications_read()— flip the read flag on the caller's OWN rows only.
--   get_caregiver_requests() — a caregiver's incoming + active reservations with
--                              the elder's FIRST NAME + district + line items +
--                              slots + totals. NEVER phone/email; the precise
--                              address is exposed only AFTER approval.
--   get_elder_reservations() — an elder's own reservations with the caregiver's
--                              public display name + the same booking details.
--
-- WHY RPCs (the one-way rule + phone privacy):
--   A caregiver has NO policy to read public.profiles, so it cannot read an
--   elder's first name with a plain client join. These functions are
--   SECURITY DEFINER and re-scope every row to `auth.uid()` (the caller is the
--   elder who owns the reservation OR the caregiver it is directed at). They
--   return ONLY the minimal, agreed fields — first name + district — and never
--   phone, email, age, or last name. There is no "browse elders" surface here.
--
-- This migration is IDEMPOTENT (create-or-replace + guarded publication add) so
-- it doubles as the runnable patch in SUPABASE_FIX_NOTIFICATIONS.sql.
